-- 案件管理システム用テーブル作成スクリプト

-- 1. projectsテーブル（案件テーブル）
...
-- 2. chat_messagesテーブル（チャットメッセージテーブル）
...
-- 3. favorite_contractorsテーブル（お気に入り受注者テーブル）
...
CREATE INDEX IF NOT EXISTS idx_favorite_contractors_org_id ON favorite_contractors(org_id);
CREATE INDEX IF NOT EXISTS idx_contracts_project_id ON contracts(project_id);
CREATE INDEX IF NOT EXISTS idx_billing_project_id ON billing(project_id);
