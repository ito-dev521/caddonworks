-- 既存のテーブル構造と互換性のあるテーブル作成スクリプト

-- 1. 既存のテーブルを安全に削除（存在する場合のみ）
...
-- 2. projectsテーブル（案件テーブル）を作成
-- 既存のusersテーブルとorganizationsテーブルを参照
...
-- 3. chat_messagesテーブル（チャットメッセージテーブル）を作成
...
-- 4. favorite_contractorsテーブル（お気に入り受注者テーブル）を作成
...
-- 7. project_attachmentsテーブル（案件添付資料テーブル）を作成
