...
CREATE TABLE IF NOT EXISTS invoices (
    ...
    system_fee DECIMAL(12,2) DEFAULT 0,
    total_amount DECIMAL(12,2) NOT NULL DEFAULT 0,
    subtotal DECIMAL(12,2), -- 旧スキーマ互換
    amount DECIMAL(12,2), -- 旧スキーマ互換
    ...
    -- ステータス
    status VARCHAR(20) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'issued', 'sent', 'paid', 'overdue', 'cancelled')),

    -- 方向・メモ
    direction VARCHAR(20), -- 'to_operator', 'to_client' など
    memo TEXT,
...
DO $$
BEGIN
    RAISE NOTICE 'invoices テーブルが正常に作成されました';
END $$;
