...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'projects.contractor_idの外部キー制約の追加に失敗しました: %', SQLERRM;
        END;
    ELSE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'projects') THEN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'bids.project_idの外部キー制約の追加に失敗しました: %', SQLERRM;
        END;
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'bids.contractor_idの外部キー制約の追加に失敗しました: %', SQLERRM;
        END;
    ELSE
        RAISE NOTICE 'usersテーブルが存在しないため、外部キー制約をスキップしました';
    END IF;
...
-- projectsテーブルの最終構造確認
SELECT 'projectsテーブルの最終構造:' as info;
